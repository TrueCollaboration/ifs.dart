...
import 'package:ifs/src/external/abstract/InputController.dart';
...
class FileInput extends InputController<Uint8List> {
  ...
  @override
  Future<int> readFrom(Uint8List buffer, int dstOffset, [int offset = 0, int length = 0]) async {
    if(length == 0)
      length = buffer.length - offset;
  ...
  @override
  Future<int> writeTo(Uint8List buffer, int dstOffset, [int offset = 0, int length = 0]) async {
    if(length == 0)
      length = buffer.length - offset;
    final tmp = this.offset;
    await _fileStream.setPosition(dstOffset);
  ...
  FutureOr<T> synchronized<T>(FutureOr<T> Function() computation, {Duration? timeout}) async {
    ...
    try {
      return await _lock.synchronized(computation, timeout: timeout ?? const Duration(seconds: 10));
    } on TimeoutException {
      debug(StackTrace.current);
      rethrow;
    }
  }
